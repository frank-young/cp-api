...
class CommentpraiseController extends Controller
{
    public function status(Request $request)
    {
      ...
      $commentpraise = Commentpraise::where(['comment_id' => $comment_id, 'openid' => $openid])->first();
      if (!empty($commentpraise)) {
        $commentpraise = ['praise_status' => 1];
      } else {
        $commentpraise = ['praise_status' => 0];
      }
      $res = returnCode(true,'查询成功', $commentpraise);
    ...
    public function agree(Request $request)
    {
      ...
      if ($result) {
        Comment::addPraiseNum($comment_id);   // 增加点赞数
    ...
    public function cancel(Request $request)
    {
      ...
      if ($result) {
        Comment::reducePraiseNum($comment_id); // 减少点赞数
